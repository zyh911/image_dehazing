...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_im = np.asarray(input_img)
        input_im = cv2.cvtColor(input_im, cv2.COLOR_RGB2YUV)
        target_im = np.asarray(target_img)
        target_im = cv2.cvtColor(target_im, cv2.COLOR_RGB2YUV)

        # CxHxW
        input_tensor = torch.from_numpy(input_im.transpose((2, 0, 1)))
        input_tensor = input_tensor.float().div(255)

        # CxHxW
        target_tensor = torch.from_numpy(target_im.transpose((2, 0, 1)))
