...
model = SuperviseModel({
    ...
    'supervise':{
        'out':  {'MSE_loss': {'obj': ChannelMSELoss(channel_weights=(0.5, 0.25, 0.25)),  'factor':1.0, 'weight':1.0}}, 
    },
    'metrics': {}
     
})

train_dataset = Photo2PhotoData_YUV({
            'data_root': DATASET_DIR,
            'desc_file_path': os.path.join(DATASET_TXT_DIR, DATASET_ID, 'train.txt'),
})

valid_dataset = Photo2PhotoData_YUV({
            'data_root': DATASET_DIR,
