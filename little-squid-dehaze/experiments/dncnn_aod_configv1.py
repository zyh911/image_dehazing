...
snapshot_path = '/root/zyh3/train_tasks/dncnn_aod_config/models/snapshot_20_G_model'
checkpoint = torch.load(snapshot_path, map_location=lambda storage, loc: storage)
...
hr_size = 128
target_net = DnCnn_AOD(layer_num=20)
target_net.load_state_dict(checkpoint)

model = SuperviseModel({
    'net': target_net, 
    'optimizer': torch.optim.Adam([{'name':'net_params', 'params':target_net.parameters(), 'base_lr':0.00005}], betas=(0.9, 0.999), weight_decay=0.001),
    'lr_step_ratio': 0.5,
    'lr_step_size': 5,
